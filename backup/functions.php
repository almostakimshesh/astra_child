...
// for user appoval start ==========================================================================

// Add a custom user role for pending approval
function add_pending_user_role() {
    add_role('pending', 'Pending Approval', []);
}
...
function restrict_pending_users($user, $username, $password) {
    if (!is_wp_error($user)) {
        ...
        if ($status === 'pending') {
            return new WP_Error('pending_approval', 'Your account is awaiting admin approval.');
        }
    }
    return $user;
}
...
// Admin can approve users in WordPress
...
// Add "Approve" button in WP Admin
function add_approve_button($actions, $user) {
    if (get_user_meta($user->ID, 'account_status', true) === 'pending') {
        $approve_url = admin_url("users.php?approve_user={$user->ID}");
        $actions['approve'] = "<a href='$approve_url'>Approve</a>";
    }
    return $actions;
}
add_filter('user_row_actions', 'add_approve_button', 10, 2);

// Process user approval when admin clicks "Approve"
function process_user_approval() {
    if (isset($_GET['approve_user']) && current_user_can('manage_options')) {
        approve_pending_user($_GET['approve_user']);
